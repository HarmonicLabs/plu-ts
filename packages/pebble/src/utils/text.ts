...
/** Tests if the specified character code is a valid hexadecimal symbol [(a-f)|(A-F)]. */
...
export function isIdentifier(str: string): boolean {
  let len = str.length;
  if (!len) return false;
  let cp = <number>str.codePointAt(0);
  if (!isIdentifierStart(cp)) return false;
  let i = numCodeUnits(cp);
  while (i < len) {
    cp = <number>str.codePointAt(i);
...
const unicodeIdentifierStart: number[] = [/*
...
const unicodeIdentifierPart: number[] = [/*
| from  ...  to | from  ...  to | from  ...  to | from  ...  to |*/
  ...
  if (level <= 4) {
    ...
    if (indentCache.size <= 1024) {
      if (indentCache.has(level)) {
        const thing = indentCache.get(level);
        if(!thing) throw new Error("indentCache.get(level) returned undefined");
        indents = thing;
      } else {
