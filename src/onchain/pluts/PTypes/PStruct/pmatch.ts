import { ConstantableStructCtorDef, RestrictedStructInstance, ConstantableStructDefinition, PStruct, isConstantableStructDefinition } from ".";
...
import { plet, perror, papp } from "../../Syntax";
...
( fieldsList: Term<PList<PData>>, ctor: CtorDef ): RawFields<CtorDef>
{
    ...
    <Fields extends (keyof CtorDef)[]>( ...fields: Fields ): {
        ...
        return ObjectUtils.defineReadOnlyProperty(
            ...
            <PExprResult extends PType>( expr: ( extracted: RestrictedStructInstance<CtorDef,Fields> ) => Term<PExprResult> ): Term<PExprResult> => {
                ...
                return plet( pindexList( data ).$( fieldsList ) ).in( elemAt =>
...
const results = callbacks.map( (cb, i) => cb( toRawFields( rawDataFields, ctors[i] ) ) );
...
function loop( partialObj: object, missingCtors: string[] )
{
    ...
    if( missingCtors.length === 1 )
    {
        ...
        return ObjectUtils.defineReadOnlyProperty(
            partialObj,
            "on" + capitalize( ctor ),
            ( cb: ( rawFields: RawFields<SDef[typeof ctor]> ) => Term<PType> ) => {
