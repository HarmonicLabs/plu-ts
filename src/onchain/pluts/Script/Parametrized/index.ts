...
import { FromPType, PrimType, data, typeExtends, unit, fn, TermType, asData, ToPType } from "../../type_system";
...
export class Precompiled<Purp extends LitteralPurpose, T extends TermType>
{
    ...
    **/
    ...
    )
    {
        ...
        {
            throw new BasePlutsError("invalid purpose passed to Precompiled constructor")
        }
...
): Precompiled<Purp, T>
{
    if( !isLitteralPurpose( purpose ) )
    {
        throw new BasePlutsError("invalid purpose passed to precompileParametrized");
    }
    ...
    if( tys.length < 3 ) throw new BasePlutsError("invalid term to precompile");
    ...
    )) throw new BasePlutsError("invalid return type for typed or untyped validator");

    const primOutT = outT[0]

    if( tys.length === 3 ) // must be redeemer validator without params
    {
        if( purpose === "spend" ) throw new BasePlutsError("invalid term for spending validator");
        ...
        if( primOutT === PrimType.Unit )
        {
            ...
            )) throw new BasePlutsError("untyped non-parametrized contract was typed");
    ...
    if( tys.length === 4 ) // either spend validator with no params or single params for rest
    {
        if( purpose === "spend" )
        {
            if( primOutT === PrimType.Unit )
            {
                ...
                )) throw new BasePlutsError("untyped non-parametrized spend contract was typed");
...
function toDataReprType( t: TermType ): TermType
{
    ...
    ) throw new BasePlutsError("lambda or delayed non representable by data");
